...
exports.getVacancyById = async (req, res) => {
  try {
    const vacnacyId = req.params.id;

    const vacancy = await Vacancy.findOne({ _id:vacnacyId }).populate("applicants").exec();
    ...
    return res.status(500).json({
      success: false,
      message: "Something went wrong while Fetching Vacany",
    });
  }
};
